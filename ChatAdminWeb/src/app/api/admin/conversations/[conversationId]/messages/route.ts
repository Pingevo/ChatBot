...
export async function GET(
  ...
  // ⚡ 1.5 fetch sub-messages สำหรับ bundle_message ที่มี bundle_message_ids
  ...
  if (allBundleIds.length > 0) {
    // fetch sub-messages จาก messages collection ด้วย message_id
    const subDocs = await msgColl
      .find({ message_id: { $in: allBundleIds } })
      .toArray();
    const subDocMap = new Map(subDocs.map((d) => [d.message_id, d]));
    // แปลง sub-docs เป็น ParsedMessage แล้วใส่ใน bundle field ของ parent
    for (const { parsed: p } of parsed) {
      if (p.bundle_message_ids && p.bundle_message_ids.length > 0) {
  ...
  // 2. รวบรวม item_ids ที่ต้อง lookup จาก product collection (item + variation_card + bundle sub-messages)
  ...
  const messages: ChatMessage[] = parsed.map(({ doc, parsed: p }) => {
    ...
    return {
      ...
      table: p.table,
      // ⚡ bundle_message — แปลง sub-messages เป็น ChatMessage[] ส่งไป frontend
