...
Usage:
    ...
    # ดูเคสเฉพาะเลข
    python scripts/check_progress.py 401 402 501 502

    # ดูช่วงเคส
    python scripts/check_progress.py 400-410

    # ดู 10 เคสล่าสุด
    python scripts/check_progress.py --last 10

    # ดูเคสที่ source = error
    python scripts/check_progress.py --errors

    # ดูเคสที่ไม่มีสินค้า (product_count = 0)
    python scripts/check_progress.py --no-products

    # ดูเคสใน batch ใด batch หนึ่ง
    python scripts/check_progress.py --batch per_product_warranty

    # ดูเคสที่ใช้เวลานาน (>10s)
    python scripts/check_progress.py --slow

    # รวมหลาย filter
    python scripts/check_progress.py --batch per_product_detail --last 20
"""
...
def load_results() -> list[dict]:
    if not RESULTS_FILE.exists():
        print("❌ ยังไม่มี results file (scripts/testQA2_results.json)")
        print("   รัน: python scripts/run_daily_tests.py ก่อน")
        sys.exit(1)
