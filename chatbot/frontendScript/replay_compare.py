...
def parse_raw_message(raw_payload, fallback_text: str) -> dict:
    ...
    # ⚡ normalize alias — เหมือน messageMediaParser.ts
    #   item_card → item, picture → image, faq_liveagents → faq_liveagent
    if msg_type == 'item_card':
        msg_type = 'item'
    elif msg_type == 'picture':
        msg_type = 'image'
    elif msg_type == 'faq_liveagents':
        msg_type = 'faq_liveagent'
    ...
    if re.search(r'\[รูปภาพ\]|\[image\]|\[picture\]', ft, re.I):
        if msg_type != 'image':
    ...
    elif re.search(r'\[item\]|\[itemid\]|\[item_card\]|\[สินค้า\]', ft, re.I):
        ...
        return {'message_type': 'notification', 'text': fallback_text or '',
                'notification_text': ''}
    elif re.search(r'\[faq_liveagent\]|\[faq_liveagents\]|\[โอนเจ้าหน้าที่\]', ft, re.I):
        return {'message_type': 'faq_liveagent', 'text': fallback_text or '(โอนไปยังเจ้าหน้าที่)',
                'notification_text': fallback_text or 'โอนไปยังเจ้าหน้าที่'}
...
def build_bot_message(parsed: dict, raw_doc: dict) -> tuple[str, str | None, list[str], str | None]:
    """คืน (message_to_bot, item_id, images, order_sn) — เหมือน messageService.toBotText + toBotImages + callBot.
    ...
    - ⚡ Phase 3C — ถ้าเป็น order card ที่มี order_sn → แปลงเป็น [order: <order_sn>]
      และส่ง order_sn เป็น field แยกด้วย (เหมือน item_id)
    ...
    order_sn = parsed.get('order_sn')  # ⚡ Phase 3C
    ...
    if mt in SYSTEM_MSG_TYPES:
        return '', None, [], None  # ข้าม system message
    ...
    if mt in ('item', 'variation_card') and item_id:
        ...
        return bot_msg, None, images, None  # ไม่ส่ง item_id แยก — ฝังใน message แล้ว

    if mt == 'order':
        # ⚡ Phase 3C — แปลงเป็น [order: <order_sn>] เหมือน toBotText
        is_placeholder = bool(_PLACEHOLDER_RE.match(text or ''))
        extra = '' if is_placeholder else (text or '').strip()
        if order_sn:
            bot_msg = f'[order: {order_sn}] {extra}'.strip() if extra else f'[order: {order_sn}]'
            return bot_msg, None, images, order_sn
        # ไม่มี order_sn → ส่ง text เดิม
        return text, None, images, None

    # text ปกติ
    return text, item_id, images, None
...
def replay_one(admin_db, prod_db, conv_id: str, verbose: bool = True,
    ...
    for i, um in enumerate(user_msgs):
        ...
        bot_msg, item_id, msg_images, msg_order_sn = build_bot_message(parsed, um)
