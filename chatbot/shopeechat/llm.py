...
def answer_general(
    ...
    # ⚡ T11 (NEW-8) — ทุก qtype: ตอบคำถามเจาะจงจาก context ห้าม dump list/ข้อความดิบ
    #   เคส QA: "CUKTECH คือ ZMI ไหม" → dump brand list; "ส่งด่วนได้ไหม" →
    #   ตอบแค่ "ทักแอดมิน" ทั้งที่ context มีเงื่อนไขจัดส่ง
    general_instruction += (
        " ตอบเฉพาะสิ่งที่ลูกค้าถามจาก context เท่านั้น "
        "ถ้าลูกค้าไม่ได้ขอ 'รายการทั้งหมด' โดยตรง (เช่น ถามว่ามีแบรนด์/หมวด X ไหม "
        "หรือถามเรื่องเฉพาะ) ห้าม list แบรนด์/หมวด/นโยบายทั้งหมดออกมา "
        "ห้ามตอบสั้นๆ ว่า 'ทักแอดมิน' ทั้งที่ context มีข้อมูลตอบคำถามได้ "
        "— ให้ตอบจาก context ก่อน แล้วค่อยแนะนำทักแอดมินเพิ่มถ้าจำเป็น "
        "ถ้า context ตอบคำถามเจาะจงไม่ได้จริงๆ ให้บอกตามจริงแล้วแนะนำทักแอดมิน"
    )
    if persona_extra:
