...
_EMBEDDINGS_PATH = Path(__file__).resolve().parent.parent / "exports" / "product_embeddings.npz"
...
def vector_search(
    ...
    try:
        import numpy as np
        from chatbot.embedding import embed_query
    except ImportError:
        return []
...
def to_product_card(doc: dict) -> dict:
    ...
    return {
        ...
        # description อาจยาวมาก ตัดไว้ 1500 ตัวอักษรต่อสินค้า พอให้ LLM เห็นเงื่อนไขเคลม
        "description_excerpt": (doc.get("description") or "")[:1500],
        "variants": [
            {
...
PRODUCT_TYPES: tuple[tuple[str, tuple[str, ...], str], ...] = (
    ...
    ("phone",
     ...
     r"\bmi\s+(?:note\s+)?\d+(?![wW])|black\s*shark\s*\d|\bpoco\b|iphone\s*\d|"
    ...
    ("smartwatch",
     ("สมาร์ทวอช", "สมาร์ทวอทช์", "สมาร์ทวอชท์", "นาฬิกาอัจฉริยะ",
      "smartwatch", "smart watch", "smartwatch", "mi band", "amazfit"),
    ...
    ("powerbank",
     ("แบตสำรอง", "พาวเวอร์แบงก์", "powerbank", "power bank", "แบตเตอรี่สำรอง",
      "พอร์เวอร์แบงค์"),
     r"(?:แบตสำรอง|แบตเตอรี่สำรอง|พาวเวอร์แบงก์|พอร์เวอร์แบงค์|"
...
def _detect_product_types_fuzzy(message: str) -> set[str]:
    ...
    for cand in candidates:
        ...
        # วิธีแก้: ถ้าความยาวต่างกันเกิน 2 เท่า ให้ threshold สูงขึ้น
...
def build_query(
    ...
    if "warranty" in intents:
        # เน้นสินค้าที่มีข้อมูล Warranty
        q["attribute_list.original_attribute_name"] = {"$regex": "warranty", "$options": "i"}
...
def fetch_products(
    ...
    if not used_vector_search and docs:
        model_tokens = _extract_model_tokens(message)
        if model_tokens:
            docs = _rerank_with_diversity(docs, model_tokens, limit=limit)
        else:
            docs = _rerank_by_promo_latest(docs, similarity_scores=None, limit=limit)

    cards = [to_product_card(d) for d in docs]
