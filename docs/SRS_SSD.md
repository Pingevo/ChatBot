...
> - `device_compat.py` — `_extract_max_wattage`, `_wattage_asc_key`, `_extract_device_token`, `_lookup_spec_db`, `_extract_product_connectors`, `_resolve_device_spec`, `_filter_compat_products`, `_apply_product_tiers`, `_device_spec_lookup`
> - `device_specs_data.py` — `DEVICE_SPECS` dict (curated charging-spec catalog ~140 devices: connector/wired_w/wireless_w/protocols/year/aliases) — แทน `_KNOWN_DEVICE_SPECS` เดิมที่ถูกลบ (2026-09-16 spec-db refactor)
...
| `_lookup_spec_db` | ~104 | **(2026-09-16 spec-db)** — ค้น spec จาก `DEVICE_SPECS` — exact → alias → substring longest-match (กัน "iphone 17" ทับ "iphone 17 pro max") — คืน `{device, connector, min_watt(=wired_w), wired_w, wireless_w, protocols, year}` หรือ None — product code/brand เดี่ยว/คำกว้าง → None | `_SPEC_INDEX` |
...
| `_resolve_device_spec` | ~658 | **(2026-09-16 spec-db)** — resolve device charging spec — priority: (1) `_lookup_spec_db` (curated DB — หลัก) (2) parse web search text (fallback) — คืน `{connector, min_watt, ...}` หรือ None — `_KNOWN_DEVICE_SPECS` ถูกลบ (DB ครอบคลุมกว่า + structured protocols) | `re`, `_lookup_spec_db` |
| `_filter_compat_products` | ~696 | **(2026-09-16 spec-db)** — CODE-level compat filter — กรองสินค้าที่ connector ไม่ตรงกับอุปกรณ์ออกก่อนส่ง LLM — priority: `_resolve_device_spec` (spec DB → web parse) → `intent_connector` (LLM guess — last resort) — sort by wattage ascending แบบ adequate-first (`_wattage_asc_key` + `device_min_watt`: ของที่ watt ≥ spec เครื่องขึ้นก่อน) — fallback: ดึงไม่ได้→คืนทั้งหมด, กรองเหลือ<2→รวม ambiguous, กรองว่าง→คืนทั้งหมด | `_resolve_device_spec`, `_lookup_spec_db`, `_extract_product_connectors`, `_extract_max_wattage`, `_wattage_asc_key` |
...
| `_lookup_spec_db(device_name)` | **(spec-db 2026-09-16)** ค้น spec จาก `DEVICE_SPECS` (device_specs_data.py — curated catalog ~140 devices) — exact → alias → substring longest-match — คืน `{device, connector, min_watt, wired_w, wireless_w, protocols, year}` | `_SPEC_INDEX`, `_SPEC_TERMS_BY_LEN` |
| `_resolve_device_spec(name, ws_extra)` | resolve spec ของอุปกรณ์ — `_lookup_spec_db` (curated — หลัก) → web search parse (fallback) | `re`, `_lookup_spec_db` |
| `_filter_compat_products(...)` | กรองตาม connector (ไม่กรอง wattage) + sort wattage asc แบบ adequate-first (`_wattage_asc_key` + `device_min_watt`) — spec source: `_resolve_device_spec` (DB→web) ก่อน → intent | ฟังก์ชันข้างบน |
...
| `_device_spec_lookup(db, req, ...)` | web-search spec + re-query DB หา compat products — target_device จาก intent → `_extract_device_token` fallback (generic, ไม่ hardcode รุ่น); resolve `device_min_watt` (`_lookup_spec_db` → intent → `_resolve_device_spec`) เติม catalog spec line + watt threshold ใน spec extra + sort re-query ด้วย `_wattage_asc_key` | `product_store`, `web_search` (lazy), `_extract_device_token`, `_lookup_spec_db` |
| `DEVICE_SPECS` (device_specs_data.py) | **(spec-db 2026-09-16)** curated charging-spec catalog — ~140 devices: connector/wired_w/wireless_w/protocols/year/aliases — iPhone ทุกรุ่น, iPad, MacBook, Samsung 5 ปี, Xiaomi/Redmi/Poco, Huawei/Honor, Oppo/OnePlus/Realme, Vivo/iQOO, Pixel, Nothing, Sony, Asus, Motorola, Infinix/Tecno, handhelds — แทน `_KNOWN_DEVICE_SPECS` เดิม (ลบแล้ว) | — |
