...
| `_lookup_spec_db(device_name)` | **(spec-db 2026-09-16, brand-guard 2026-09-18)** ค้น spec จาก `DEVICE_SPECS` (device_specs_data.py — curated catalog ~140 devices) — exact → boundary substring longest-match (`_term_boundary_match` กัน term ฝังใน product code) → brand guard (`_device_brand_hint` รับเฉพาะ entry แบรนด์ตรงกับ input) — คืน `{device, connector, min_watt, wired_w, wireless_w, protocols, year}` | `_SPEC_INDEX`, `_device_brand_hint`, `_spec_brand` |
