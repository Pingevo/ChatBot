...
| `get_latest_suggestion_batch` | ~315 | **2026-09-17** — ดึง suggestion batch ล่าสุด (trailing run ของ non-anchor entries ท้าย list = เทิร์นเดียวกันที่ bot แนะนำ) → list ของ cards ใหม่→เก่า; [] ถ้า tail เป็น anchor หรือ batch <2 — ใช้ใน comparison follow-up เมื่อ anchor <2 (ลูกค้าเทียบของที่ bot เพิ่งแนะนำ ไม่ใช่ anchor) |
...
| `attach_listing_fields(unit_docs)` | batch join `ShpProducts` ด้วย `item_id` (int ทั้งสองฝั่ง) — เติม `_listing` {condition, weight, dimension, short_link, promotion, is_flash_sale, image, **item_status, stock_info_v2, model.model_id/model_status/stock_info_v2**}; runtime join เพราะ promo/status/stock เปลี่ยนบ่อย (build-time copy จะ stale) | list[unit doc] | docs เดิม (เติม `_listing`) |
