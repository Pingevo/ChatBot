...
Usage:
    ...
    python test/testQA2.py --generate

    # รัน test จริง (ส่งคำถามไป /chat) — รันเมื่อผู้ใช้บอกเท่านั้น
    python test/testQA2.py --run --batch per_product_detail
    python test/testQA2.py --run --batch all
"""
...
# ---- path setup (รันจาก root หรือ test/ ก็ได้) ----
...
PLAN_FILE = ROOT / "docs" / "test" / "results" / "testQA2_plan.json"
RESULTS_FILE = ROOT / "docs" / "test" / "results" / "testQA2_results.json"
WRONG_FILE = ROOT / "docs" / "test" / "results" / "testQA2_wrong.json"
SUMMARY_FILE = ROOT / "docs" / "test" / "results" / "testQA2_summary.json"
...
def run_tests(
    ...
    for idx, case in enumerate(pending):
        if req_count >= max_reqs:
            ...
            print(f"   คำสั่งรันต่อ: python test/testQA2.py --run --batch {batch_name} --max-reqs {max_reqs} --resume")
            break
    ...
    if total - len(results) > 0:
        print(f"\n📌 รันต่อ: python test/testQA2.py --run --batch {batch_name} --max-reqs {max_reqs} --resume")
    print(f"{'='*60}")
...
def main():
    ...
    if args.stats_only:
        if not plan_path.exists():
            print(f"ไม่พบ plan file: {plan_path}")
            print("รัน: python test/testQA2.py --generate ก่อน")
            return
        plan = load_plan(plan_path)
        print("\n📊 Stats ของ test plan:")
    ...
    print(f"\nพร้อมรัน? ใช้คำสั่ง:")
    print(f"  python test/testQA2.py --run --batch all --max-reqs 500")
    print(f"  python test/testQA2.py --run --batch all --max-reqs 500 --resume  # รันต่อ")
    print(f"  python test/testQA2.py --progress  # ดูความคืบหน้า")
